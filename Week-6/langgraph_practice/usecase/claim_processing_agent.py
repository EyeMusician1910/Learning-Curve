...
import uuid
try:
    from usecase.db import initialize_database, get_connection
...
loader = TextLoader(str(BASE_DIR / "insurance_data.txt"))
...
def store_claim(state: ClaimState):
    with get_connection() as conn:
        conn.execute(
            "insert into claims (claim_id, patient_id, status, decision_details) values (?, ?, ?, ?)",
            (
                str(uuid.uuid4()),
