...
#Define the structure of  i/p state

class SupportRequest(TypedDict):
    message:str
    priority: int #Based on the priority,we will handle the request accordingly
    
#Function to categorize the support the request

def categorize_request(request: SupportRequest):
    print(f"Received Request : {request}" )
    #Implementing the Conditional Routing
...
def handle_urgent(request: SupportRequest):
    print(f"Routing to the Urgnet Suppport Team : {request} \n" ) 
    return request

#Function to process standard requests

def handle_standard(request: SupportRequest):
    print(f"Routing to the Standard Support Team : {request}")
    return request
