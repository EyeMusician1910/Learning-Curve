...
from FastAPI_with_SQLmodel.api.dependencies import ServiceDep
...
async def get_shipment(id: int,session: SessionDep,service:ServiceDep) :#Not type hinting as it's already specified in the response_model parameter #->dict[str,str | int | float] #Type hinting so that the the function takes int as an input and also hinting that the output should be dict which have the key and value of datatype string or integer.
    session
